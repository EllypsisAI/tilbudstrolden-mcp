...
/**
 * Build a fresh `McpServer` with every tool and prompt registered.
 *
 * Returns a new instance each call so multiple transports (stdio, stateless
 * HTTP-per-request) can each own their server without contending for the
 * `_transport` slot.
 */
